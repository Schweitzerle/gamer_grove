// ==================================================
// GAME ENGINE PAGINATED GAMES SCREEN
// ==================================================

// lib/presentation/pages/gameEngine/game_engine_paginated_games_screen.dart
...
class GameEnginePaginatedGamesScreen extends StatefulWidget {
  ...
  @override
  State<GameEnginePaginatedGamesScreen> createState() =>
      _GameEnginePaginatedGamesScreenState();
}

class _GameEnginePaginatedGamesScreenState
    extends State<GameEnginePaginatedGamesScreen> {
  late ScrollController _scrollController;
  late TextEditingController _searchController;
  bool _isGridView = true;
  String _searchQuery = '';

  // Available sort options for GameEngine games (nur IGDB-basierte Sortierungen)
  static const List<GameSortBy> _availableSortOptions = [
    GameSortBy.name, // Alphabetisch
    GameSortBy.ratingCount, // Rating Count (Standard)
    GameSortBy.rating, // Game Rating
    GameSortBy.releaseDate, // Release Date
    GameSortBy.aggregatedRating, // Critic Rating
  ];
  ...
  void _onScroll() {
    if (_isNearBottom) {
      context.read<GameEngineBloc>().add(const LoadMoreGameEngineGamesEvent());
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: _buildAppBar(),
      body: BlocBuilder<GameEngineBloc, GameEngineState>(
        builder: (context, state) {
          if (state is GameEngineGamesLoading) {
            return _buildLoadingState();
          } else if (state is GameEngineGamesLoaded) {
            return _buildLoadedState(state);
          } else if (state is GameEngineGamesError) {
  ...
  PreferredSizeWidget _buildAppBar() {
    return AppBar(
      ...
      title: Column(
        ...
        children: [
          Text(
            widget.gameEngineName,
            style: Theme.of(context).textTheme.titleLarge,
          ),
          BlocBuilder<GameEngineBloc, GameEngineState>(
            builder: (context, state) {
              if (state is GameEngineGamesLoaded) {
                return Text(
  ...
  Widget _buildErrorState(GameEngineGamesError state) {
    return Center(
      child: Padding(
  ...
  Widget _buildLoadedState(GameEngineGamesLoaded state) {
  ...
  Widget _buildGridView(GameEngineGamesLoaded state) {
  ...
  Widget _buildListView(GameEngineGamesLoaded state) {
    // Don't show "load more" when searching
    final showLoadMore = state.hasMore && _searchQuery.isEmpty;
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            Text(
              'No games were built with ${widget.gameEngineName}',
              style: Theme.of(context).textTheme.bodyMedium,
              textAlign: TextAlign.center,
            ),
          ],
  ...
  void _showSortDialog() {
    final currentState = context.read<GameEngineBloc>().state;
